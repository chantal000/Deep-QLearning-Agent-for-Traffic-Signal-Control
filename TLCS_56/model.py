...
class VanillaTrainModel(TrainModel):
    ...
    def _build_model(self):
        ...
        c1 = layers.Conv2D(filters = 16, kernel_size = 2, strides = (2,2), padding = "same", activation = 'relu')(conv_inputs)
        c2 = layers.Conv2D(filters = 32, kernel_size = 2, strides = (2,2), padding = "same", activation = 'relu')(c1)
        c3 = layers.Conv2D(filters = 64, kernel_size = 1, strides = (1,1), padding = "same", activation = 'relu')(c2)
...
class RNNTrainModel(TrainModel):
    ...
    def _build_model(self):
        ...
        #conv layers        
        c1 = layers.TimeDistributed(layers.Conv2D(filters = 16, kernel_size = 2, strides = (2,2), padding = "same", activation = 'relu'))(conv_inputs)
        c2 = layers.TimeDistributed(layers.Conv2D(filters = 32, kernel_size = 2, strides = (2,2), padding = "same", activation = 'relu'))(c1)
        c3 = layers.TimeDistributed(layers.Conv2D(filters = 64, kernel_size = 1, strides = (1,1), padding = "same", activation = 'relu'))(c2)
