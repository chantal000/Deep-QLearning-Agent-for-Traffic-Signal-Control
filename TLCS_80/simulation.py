...
class TrainSimulation(Simulation):
    def __init__(self, Model, TargetModel, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_actions, training_epochs, copy_step, update_every_x_episodes):
        ...
        self._Memory = Memory
        self._gamma = gamma
        self._update_every_x_episodes = update_every_x_episodes
        self._reward_store = []
        ...
        self.testing_delay_store = []
        self.testing_wait_store = []
        self.testing_queue_store = []
    ...
    def _save_greedy_episode_stats(self):
        ...
        """
        self.testing_reward_store.append(self._greedy_reward_list)  # how much negative reward in this episode
        self.testing_delay_store.append(self._greedy_delay_list)
        self.testing_wait_store.append(self._greedy_wait_list)
        self.testing_queue_store.append(self._greedy_queue_list)
...
class VanillaTrainSimulation(TrainSimulation):
    def run(self, episode, epsilon):
        ...
        while self._step < self._max_steps:
            ...
            current_total_reward = -1 * self._get_vehicle_delay()
            # reward = old_total_reward - current_total_reward

            # saving the data into the memory
            if self._step != 0:
                self._Memory.add_sample((old_state, old_action, current_total_reward, current_state))
            ...
            self._sum_neg_reward += current_total_reward
        ...
        if episode % self._update_every_x_episodes == 0:
            print("Testing ...")
    ...
    def _greedy_run(self,episode):
        ...
        self._greedy_reward_list = []
        self._greedy_delay_list = []
        self._greedy_wait_list = []
        self._greedy_queue_list = []
        ...
        for scenario_number in range(4):
            ...
            while self._step < self._max_steps:
                ...
                self._sum_delay += -1 * current_total_reward
                self._sum_waiting_time += self._get_waiting_times()
                self._sum_queue_length += self._get_queue_length()
            
            # save KPI in list
            self._greedy_reward_list.append(self._sum_neg_reward)
            self._greedy_delay_list.append(self._sum_delay)
            self._greedy_wait_list.append(self._sum_waiting_time)
            self._greedy_queue_list.append(self._sum_queue_length)
...
class RNNTrainSimulation(TrainSimulation):
    def __init__(self, Model, TargetModel, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_actions, training_epochs, copy_step, update_every_x_episodes, PredictModel):
        self._PredictModel = PredictModel
        super().__init__(Model, TargetModel, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_actions, training_epochs, copy_step, update_every_x_episodes)
    ...
    def run(self, episode, epsilon):
        ...
        while self._step < self._max_steps:
            ...
            # saving the data into the memory
            if self._step != 0:
                self._Memory.add_to_buffer((old_state, old_action, current_total_reward, current_state))
    ...
    def _choose_greedy_action(self, state):  #TODO
        """
        Do an exploitative action
        """
        return np.argmax(self._PredictModel.predict_one(state)) # the best action given the current state
