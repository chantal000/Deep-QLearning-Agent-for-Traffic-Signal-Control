...
class TestModel:
    ...
    def _load_my_model(self, model_folder_path):
        ...
        if os.path.isfile(model_file_path):
            ...
            #check if it is a recurrent model, if yes, we have to make it stateful 
            # to do this, we build a new model with a batch input of 1
            if len(loaded_model.layers[0].input.shape) > len(self._state_shape)+1:
                loaded_model = self._build_recurrent_model(loaded_model)
    ...
    def _build_recurrent_model(self, loaded_model):
        ...
        output_shape = loaded_model.layers[-1].output.shape[2]
        ...
        all_combined = layers.concatenate([elapsed_time_inputs, flat])
        lstm = layers.LSTM(96, activation='tanh', return_sequences=True, stateful = True)(all_combined)
        dense = layers.Dense(32, activation='relu')(lstm)
        dense = layers.Dense(16, activation='relu')(dense)
        outputs = layers.Dense(output_shape, activation='linear')(dense)
        ...
        #LOAD THE CORRECT WEIGHTS INTO THE MODEL
