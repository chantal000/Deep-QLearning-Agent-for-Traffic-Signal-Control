...
class RNNTrainSimulation(TrainSimulation):
    def __init__(self, Model, TargetModel, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs, copy_step, PredictModel):
        self._PredictModel = PredictModel
        super().__init__(Model, TargetModel, Memory, TrafficGen, sumo_cmd, gamma, max_steps, green_duration, yellow_duration, num_states, num_actions, training_epochs, copy_step)
    ...
    def run(self, episode, epsilon):
        ...
        while self._step < self._max_steps:
            ...
            #TO DO: NOT NECESSARY, CAN BE REMOVED (ONE OF THE TWO)
            # saving the data into the memory
            if self._step != 0:
                if isinstance(self._Memory, NormalMemory):
                    self._Memory.add_sample((old_state, old_action, reward, current_state))
