...
class Visualization:
    ...
    def training_save_data_and_plot_greedy_episodes(self, data, filename, xlabel, ylabel, episode_update_interval):
        """
        Produce a plot of performance of the agent over the session and save the relative data to txt
        """
        with open(os.path.join(self._path, 'plot_'+filename + '_data.txt'), "w") as file:
        ...
        episodes_array = np.arange(0,number_runs*episode_update_interval,episode_update_interval)    #may need to change the value!
